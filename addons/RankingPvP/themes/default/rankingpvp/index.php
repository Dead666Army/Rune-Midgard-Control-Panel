...
<form action="" method="get" class="search-form2">
	<?php echo $this->moduleActionFormInputs('rankingpvp', 'index') ?>
	<p>
...
<table class="horizontal-table">
	...
	<tr<?php if (!isset($chars[$i])) echo ' class="empty-row"'; if ($i === 0) echo ' class="top-ranked" title="'. sprintf(Flux::message('RankingPopup'),htmlspecialchars($chars[$i]->char_name),$topRankType).'"' ?>>
		...
		<?php endif ?>
		<td><?php echo number_format($chars[$i]->kill) ?></td>
		<td><?php echo number_format($chars[$i]->death) ?></td>
		<td><?php echo number_format($chars[$i]->kdr) ?></td>
		<?php else: ?>
		<td colspan="8"></td>
		<?php endif ?>
	</tr>
