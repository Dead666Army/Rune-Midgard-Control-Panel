...
<h2><?php echo htmlspecialchars(Flux::message('RankingDeath')) ?></h2>
<h3>
	<?php echo htmlspecialchars(sprintf(Flux::message('RankingDeathTop'), number_format($limit=(int)Flux::config('DeathRankingLimit')))) ?>
...
<table class="horizontal-table">
	<tr>
		...
		<th><?php echo Flux::message('RankingDeathLabel')?></th>
	...
	<tr<?php if (!isset($chars[$i])) echo ' class="empty-row"'; if ($i === 0) echo ' class="top-ranked" title="'. sprintf(Flux::message('RankingDeathPopup'),htmlspecialchars($chars[$i]->char_name),$topRankType).'"' ?>>
